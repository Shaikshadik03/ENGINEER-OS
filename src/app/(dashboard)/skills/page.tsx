...
const SKILL_TREE: SkillNode[] = [
  // ── 1. PROGRAMMING FUNDAMENTALS ──
  {
    ...
    modules: [
      ...
      {
        id: 'py-mod-3',
        title: 'Module 3: Async Concurrency & FastAPI',
  ...
  },
  {
    ...
    modules: [
      ...
      {
        id: 'c-mod-2',
        title: 'Module 2: Pointers, Structs & Malloc',
  ...
  },
  {
    ...
    modules: [
      ...
      {
        id: 'dsa-mod-2',
        title: 'Module 2: Dynamic Programming & Trees',
  ...
  // ── 2. CORE CS SUBJECTS ──
  {
    ...
    modules: [
      {
        id: 'db-mod-1',
        title: 'Module 1: Queries, Filtering & Aggregations',
      ...
      {
        id: 'db-mod-2',
        title: 'Module 2: JOINs, Indexing & ACID Transactions',
  ...
  // ── 3. DEV TOOLS ──
  {
    ...
    modules: [
      {
        id: 'git-mod-1',
        title: 'Module 1: Commits, Branches & Remote Push',
  ...
  // ── 4. WEB DEV ──
  {
    ...
    modules: [
      {
        id: 'react-mod-1',
        title: 'Module 1: JSX, Components & useState',
...
export default function GamifiedSkillsTreePage() {
  ...
  const [expandedSkillIds, setExpandedSkillIds] = useState<Set<string>>(new Set(['python-mastery']))
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-16 animate-in fade-in duration-500">
        ...
        <div className="space-y-2 max-w-xl">
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-extrabold bg-indigo-500/10 border border-indigo-500/30 text-indigo-400 px-3 py-1 rounded-full uppercase tracking-wider flex items-center gap-1">
              <Sparkles size={12} /> Direct Inline Curriculum
            </span>
          ...
          <p className="text-xs text-gray-400 leading-relaxed">
            Click any skill to expand Modules & Lessons directly inline on the page — 0 popup modals, 0 new tabs!
          </p>
        </div>
      ...
      {/* ── INLINE EXPANDABLE SKILLS LIST (DIRECT ON PAGE!) ── */}
      <div className="space-y-5">
        {filteredSkills.map(skill => {
          ...
          return (
            <div
              key={skill.id}
              className={`bg-[#111118] border rounded-2xl transition-all overflow-hidden ${
                isExpanded ? 'border-indigo-500/50 bg-indigo-500/2 ring-1 ring-indigo-500/20' : 'border-white/10 hover:border-white/20'
              }`}
            >
              {/* Skill Card Header (Click to Expand Inline!) */}
              <div
                onClick={() => toggleSkillExpand(skill.id)}
                className="p-5 cursor-pointer flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white/2 hover:bg-white/4 transition-colors"
              >
                <div className="flex items-center gap-4">
                  <span className="text-3xl p-2.5 bg-white/5 rounded-2xl border border-white/10 shrink-0">{skill.icon}</span>
                  <div>
                    <div className="flex items-center gap-2">
                      <h2 className="text-base font-bold text-white group-hover:text-indigo-300 transition-colors">
                        {skill.name}
                      </h2>
                      <span className="text-[10px] font-bold text-indigo-400 bg-indigo-500/10 border border-indigo-500/20 px-2.5 py-0.5 rounded">
                        {skill.domain}
                      </span>
                    </div>
                    <p className="text-xs text-gray-400 mt-1 max-w-2xl">{skill.description}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 shrink-0 self-end md:self-center">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-bold text-emerald-300 bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-1 rounded-lg flex items-center gap-1">
                      <BookOpen size={12} /> {totalLessonsCount} Lessons
                    </span>
              ...
              {/* INLINE MODULES & LESSONS (NO MODALS, NO POPUPS!) */}
              {isExpanded && (
                <div className="p-6 border-t border-white/10 space-y-6 bg-black/20 animate-in slide-in-from-top-2 duration-300">
                  {skill.modules.map((mod, modIdx) => {
                    ...
                    return (
                      <div key={mod.id} className="bg-white/3 border border-white/8 rounded-2xl p-5 space-y-4">
                        ...
                        <div className="flex justify-between items-start border-b border-white/8 pb-3">
                          <div>
                            <span className="text-[10px] font-extrabold text-indigo-400 uppercase tracking-wider">MODULE {modIdx + 1}</span>
                            <h3 className="text-sm font-bold text-white mt-0.5 flex items-center gap-2">
                              {isChallengeDone ? <CheckCircle2 size={16} className="text-emerald-400" /> : <Layers3 size={16} className="text-indigo-400" />}
                              {mod.title}
                            </h3>
                        ...
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between bg-indigo-500/10 border border-indigo-500/20 p-4 rounded-xl gap-4">
                          ...
                          <button
                            onClick={() => {
                              setActiveChallenge({ challenge: mod.challenge, skillId: skill.id })
